<#
.SYNOPSIS
    Uninstalls all old versions of the Az meta-module and every Az.* sub-module,
    keeping only the latest version that is currently installed on the machine.

.DESCRIPTION
    For each installed module whose name is "Az" or starts with "Az.":
        1. Find every installed version.
        2. Identify the highest version (latest already installed).
        3. Uninstall every other version.

    Run from an elevated PowerShell session if the modules live under a system path
    (e.g. C:\Program Files\WindowsPowerShell\Modules). Close all other PowerShell
    sessions before running so the module files are not locked.
...
.EXAMPLE
    .\Uninstall-AzOldVersions.ps1 -WhatIf

.EXAMPLE
    .\Uninstall-AzOldVersions.ps1 -Force
#>
...
# --- Collect installed Az / Az.* modules ------------------------------------
...
if (-not $installed) {
    Write-Host 'No Az or Az.* modules are installed. Nothing to do.' -ForegroundColor Yellow
    return
}
...
if (-not $toRemove) {
    Write-Host 'Only the latest version of each Az / Az.* module is installed. Nothing to remove.' -ForegroundColor Green
    return
}
...
$toRemove | ForEach-Object {
    Write-Host ('  {0,-30} keep {1,-12} remove {2}' -f $_.Name, $_.KeepVersion, ($_.RemoveVersions -join ', '))
}
...
elseif (-not $Force) {
    ...
    do {
        $reply = $Host.UI.PromptForChoice(
            'Confirm uninstall',
            'Do you want to continue and uninstall the old Az / Az.* versions listed above?',
            $choices,
            1  # default = No
        )
...
# --- Uninstall sub-modules first, Az last -----------------------------------
$results = New-Object System.Collections.Generic.List[object]
$ordered = @($toRemove | Where-Object Name -ne 'Az') + @($toRemove | Where-Object Name -eq 'Az')
...
foreach ($entry in $ordered) {
    foreach ($ver in $entry.RemoveVersions) {
        ...
        Write-Progress -Activity 'Uninstalling old Az modules' `
...
Write-Progress -Activity 'Uninstalling old Az modules' -Completed
