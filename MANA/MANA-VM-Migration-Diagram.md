...
> Background: [How Accelerated Networking, VFs, Mellanox, and MANA actually work](./MANA-AccelNet-VF-Primer.md)

> Compiled from publicly available Microsoft documentation and product announcements (Azure Infrastructure Blog +
> Microsoft Learn); a personal reference, not an official Microsoft communication. Networking limits are tied to the
> **VM size, not the hardware** — the VM size stays the same; only the underlying host generation changes.
...
flowchart TD
    ...
    Land --> OS{"MANA driver present<br/>in the guest image?<br/>(not just a supported OS version)"}
...
- **NVAs or VMs** can pin to non-MANA hardware with the **`LegacyVMNVA`** tag until **31 May 2027**, after which it is no
  longer honored. The tag must be **active before the VM is placed** on MANA hardware — applying it afterward does not
  move an already-placed VM back. Activate it with a **`reapply`** operation (no downtime), not deallocate/start.
